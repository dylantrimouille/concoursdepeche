...
public function createOrganizer(){
    ...
    $sql = 'INSERT INTO `organizers` (`lastname`, `firstname`, `pseudo`, `phone`, `email`, `password`, `validated_token`, `role_id`)
...
public static function deleteToken($id){
    $sql = 'UPDATE `organizers` SET `validated_token`= null
            WHERE `user_id` = :user_id;';
    try {
        $pdo = Database::getInstance();
        $sth = $pdo->prepare($sql);
...
public static function setValidateAccount($id){
    $sql = 'UPDATE `organizers` SET `validated_at`= CURRENT_TIMESTAMP()
            WHERE `user_id` = :user_id;';
...
public static function isValidated($email){
    $sql = 'SELECT `validated_at` FROM `organizers` WHERE `email` = :email;';
...
public static function read($search = NULL){
    ...
    $sql = 'SELECT * FROM `organizers` WHERE `lastname` LIKE :search OR `firstname` LIKE :search ;';
...
public function update($id){
    try{
        ...
        if(!$this->isValidated($this->_mail) || $this->_mail==$response->mail){
            $sql = 'UPDATE `organizers` SET `lastname` = :lastname, `firstname` = :firstname, `pseudo` = :pseudo, `phone` = :phone, `mail` = :mail
                    WHERE `user_id` = :user_id;';
